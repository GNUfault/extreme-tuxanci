...
extern sockaddr_in clientInfo;

/* new client is connected, we need create new structure  */
bool client_connected (unsigned int client)
{
	if (client > 0) {
...
bool client_disconnected (client_t *client)
{
...
/* register new server  */
bool server_register (client_t *client, char *buf, unsigned int size)
{
...
/* register new server  */
bool server_getlist (client_t *client, char *buf, unsigned int size)
{
...
bool server_add (unsigned ip, unsigned port)
{
	server_t *ctx;
...
bool handler (client_t *client, char *buf, int size)
{
	if (size < 1)
		return 0;
...
bool init_resolve ()
{
	...
	/** PRESET server 2 */
	server_add (NET_IPV4_TO_ADDR (193, 85, 244, 165), 2200);

	/** PRESET server 3 */
	server_add (NET_IPV4_TO_ADDR (77, 78, 118, 22), 2200);
