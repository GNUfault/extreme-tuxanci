...
static void
action_saveTux(space_t * space, tux_t * tux, textFile_t * textFile)
{
   char str[STR_PROTO_SIZE];

   snprintf(str, STR_PROTO_SIZE,
            "TUX %d %d %d %d %d %d %d %d %s %d %d %d %d %d %d %d %d %d %d %d",
            tux->id, tux->x, tux->y, tux->status, tux->position, tux->control,
            tux->frame, tux->score, tux->name, tux->gun, tux->bonus,
...
static void
saveContextArenaToTextFile(textFile_t * textFile, arena_t * arena)
{
   char str[STR_PROTO_SIZE];
...
void
saveArena(char *filename, arena_t * arena)
{
   textFile_t *textFile;
...
static void
loadTuxFromLine(char *line, arena_t * arena)
{
...
static void
loadShotFromLine(char *line, arena_t * arena)
{
   char cmd[STR_PROTO_SIZE];
...
static void
loadItemFromLine(char *line, arena_t * arena)
{
   char cmd[STR_PROTO_SIZE];
...
static arena_t *
loadContextArenaFromTextFile(textFile_t * textFile)
{
   arena_t *arena;
   int i;
