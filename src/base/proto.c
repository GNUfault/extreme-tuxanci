...
proto_recv_hello_server(client_t * client, char *msg)
{
   ...
   if (supportVersion == NULL) {
      supportVersion =
         getSetting("SUPPORT_CLIENTS", "--support-clients", TUXANCI_VERSION);
   }
#endif
...
proto_send_newtux_server(int type, client_t * client, tux_t * tux)
{
   ...
   snprintf(msg, STR_PROTO_SIZE,
            ...
            tux->id, x, y, tux->status, tux->position, tux->frame, tux->score,
            tux->name, tux->gun, tux->bonus, tux->shot[GUN_SIMPLE],
            tux->shot[GUN_DUAL_SIMPLE], tux->shot[GUN_SCATTER],
            tux->shot[GUN_TOMMY], tux->shot[GUN_LASSER], tux->shot[GUN_MINE],
