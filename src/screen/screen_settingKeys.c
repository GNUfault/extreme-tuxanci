...
setKeytableFromConfigFile(textFile_t * configFile) 
{
   ...
   loadValueFromConfigFile(configFile, "TUX_RIGHT_MOVE_RIGHT", val, STR_SIZE,
                            "275");
...
initScreenSettingKeys() 
{
      ...
      newWidgetLabel(_("Left:"), tux_left_keyup->x, tux_left_keydown->y + 20,
                     WIDGET_LABEL_LEFT);
   tux_left_keyright =
      newWidgetLabel(_("Right:"), tux_left_keyup->x, tux_left_keyleft->y + 20,
                     WIDGET_LABEL_LEFT);
      ...
      newWidgetLabel(_("Switch gun:"), WINDOW_SIZE_X / 2 + tux_left_keyup->x,
                     tux_left_keyfire->y + 20, WIDGET_LABEL_LEFT);
